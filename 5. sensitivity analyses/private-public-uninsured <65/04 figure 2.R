...
results6 <- read.csv("./results.6h.priv.unins-LT65.csv") %>%
...
results12 <- read.csv("./results.12h.priv.unins-LT65.csv") %>%
  select(Percentile, ROR, ROR.lower, ROR.upper) %>%
...
results6 <- read.csv("./results.6h.pub.unins-LT65.csv") %>%
  select(Percentile, ROR, ROR.lower, ROR.upper) %>%
  mutate(Window = "6 hours") %>%
  mutate(Percentile = Percentile*100)
...
results12 <- read.csv("./results.12h.pub.unins-LT65.csv") %>%
  select(Percentile, ROR, ROR.lower, ROR.upper) %>%
  mutate(Window = "12 hours") %>%
  mutate(Percentile = Percentile*100)
...
results6 <- read.csv("./results.6h.pub.priv-LT65.csv") %>%
  select(Percentile, ROR, ROR.lower, ROR.upper) %>%
  mutate(Window = "6 hours") %>%
  mutate(Percentile = Percentile*100)
...
results12 <- read.csv("./results.12h.pub.priv-LT65.csv") %>%
  select(Percentile, ROR, ROR.lower, ROR.upper) %>%
  mutate(Window = "12 hours") %>%
  mutate(Percentile = Percentile*100)
...
plot_row <- plot_grid(a,b,c, ncol = 3)
title <- ggdraw() + draw_label("< 65 years")


d <- plot_grid(title, plot_row, rel_heights = c(0.1, 1), ncol = 1)


ggsave(file= "figure1_sens-LT65.pdf", d , width = 11, height = 8.5)
