...
unins.pred.12hr <- read.csv("pred_unins_data_12-LT65.csv")
priv.pred.12hr <- read.csv("pred_priv_data_12-LT65.csv")
...
#join together
predictions<- full_join(unins.pred.12hr, priv.pred.12hr)


#generate predictions
#alpha-- subtact log )) in group 2 from group 1
alpha <- unins.pred.12hr$log.fit.or - priv.pred.12hr$log.fit.or

#se(alpha) -- sqrt(se1^2 + se2^2)
se.alpha <- sqrt(unins.pred.12hr$log.se**2 + priv.pred.12hr$log.se**2)
...
write.csv(strat_results_12h, "results.12h.priv.unins-LT65.csv")
